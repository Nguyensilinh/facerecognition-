...
now = datetime.now()
currentTime = now.strftime("%Hh%Mm")

today = date.today()
currentDay = today.strftime("%d_%m_%Y")

attenResults = "attenResult.csv"
with open(attenResults, "a") as file:
...
def index():
    ...
    if request.method == "GET":
        ...
        if len(os.listdir("AttendanceData/02_08_2022"))>0:
            for image_path in os.listdir("AttendanceData/02_08_2022"):
                print(image_path)
            #Tiến hành thao tác nhận diện
                img_org = cv2.imread("AttendanceData/02_08_2022/"+image_path)
                ...
                os.remove("AttendanceData/02_08_2022/"+image_path)
                if results:
                # colors = [(0,255,0), (0,0,255),(255,0,0),(255,255,0),(0,255,255)]
                    ...
                    for i in range(len(results)):
                        ...
                        #color = (int(colors[class_index][0]), int(colors[class_index][1]), int(colors[class_index][2]))
                            ...
                            if class_probability > 95:
                                ...
                                with open(attenResults, "a") as file:
                                    file.write(predict_name[0] +","+ currentTime + "\n")
                                if not os.path.exists("static/" + currentDay):
                                    os.mkdir("static/" + currentDay)
                                cv2.imwrite(os.path.join("static/" + currentDay, str(predict_name[0]) + currentTime + ".jpg"),img_org)

                                return render_template("diemdanh.html", imgname=currentDay + "/"+str(predict_name[0]) + currentTime + ".jpg",
                                                                facename=predict_name[0],
                                                                # probability=class_probability,
                                                       mess = "Nhận diện thành công",
                                                                success=True)
                            else:
                                cv2.putText(img_org, "Unknown", (x1, y1),
                                        cv2.FONT_HERSHEY_COMPLEX, 1, color, 2)

                                return render_template("diemdanh.html", imgname="Fail.jpg",
                                                                mess="Không nhận diện được khuôn mặt trong ảnh",
                                                                lowProbability = True)
                    ...
                    return render_template("diemdanh.html",  imgname="AttendanceData/02_08_2022/"+image_path,
                                           mess = "Không phát hiện được khuôn mặt trong ảnh",
                                                        fail = True)
        else:
            return render_template("diemdanh.html", imgname="Done.jpg",
                                   mess = "Đã điểm danh xong",
                                   done=True)
